...
# initialize n single-node trees containing the symbols of the alphabet given,
# and compute the frequency of each symbol and record them into the node
# create a priority queue for saving nodes
# for each iteration, we pop two nodes with the minimal frequency out of the queue,
# replacing them with a new node whose frequency is the sum of those two
# and set the left child and right child to those two nodes respectively
# the loop is over until a single tree is obtained
# for Huffman code, starting from the Huffman tree's root node,
# the codeword is obtained by searching symbol leaf node from the root node
# and append 0 if the child is the left child, otherwise, append 1.
...
print("The original string character is " + string)
